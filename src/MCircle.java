...
public class MCircle implements IShape{
    double x,y,r,thickness;
    int mythick,mystyle;
    ...
    public void draw(GraphicsContext g) {
        ...
        g.setEffect(null);
        g.fillOval(x*ratiox+thickness/2,y*ratioy+thickness/2,r*ratiox-thickness,r*ratioy-thickness);
        if(selected){
            g.setEffect(new DropShadow(5,Color.RED));
        } else g.setEffect(null);
        g.strokeOval(x*ratiox,y*ratioy,r*ratiox,r*ratioy);
        g.setEffect(null);
        g.beginPath();
    }

    public boolean ifcontains(Point2D point){
        Ellipse check = new Ellipse(x*ratiox+r*ratiox/2, y*ratioy+r*ratioy/2,r/2*ratiox,r/2*ratioy);
    ...
    MCircle(double _x,double _y, double _r, Color linec, Color fillc, int t, int s, double _stagewidth, double _stageheight) {
        x = _x;
        y = _y;
        r = _r;
        redl=linec.getRed();
